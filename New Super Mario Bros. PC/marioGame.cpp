...
void CMarioGame::LoadResources(LPDIRECT3DDEVICE9 d3ddv)
{
	...
	int mario_x = 20;
	int mario_y = GROUND_Y;

	_IsOnGround = true;

	int mario_vx = 0;
	int mario_vx_last = 1.0f;
	int mario_vy = 0;
	int mario_width = 1;
	int mario_height = 1;
	

	mario = new Mario(mario_x, mario_y, mario_width, mario_height, mario_vx, mario_vy, mario_vx_last, mario_right);
...
void CMarioGame::ProcessInput(LPDIRECT3DDEVICE9 d3ddv, int t)
{
	if (IsKeyDown(DIK_RIGHT))
	{
		...
		else
		{
			...
			if (mario_vx > 0)//mô phỏng gia tốc giảm dần
			{
				mario_vx -= 0.0000003f;//giảm vận tốc khi từ từ khi mario dừng
				if (mario_vx < 0 || mario_vx < 0.00005f)// cho đến khi vận tốc giảm gần về 0 thì dừng hẳn set vx=0;
					mario_vx = 0;
			}
			if (mario_vx < 0)//mô phỏng gia tốc giảm dần 
			{
